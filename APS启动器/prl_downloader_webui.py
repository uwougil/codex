...
def default_output_dir() -> str:
    return str((Path.home() / "Desktop" / "aps-condmat-downloads").resolve())


def default_user_data_dir() -> str:
    return str((Path(tempfile.gettempdir()) / "aps_scrapling_profile").resolve())
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the local web UI for the APS condensed-matter downloader.")
...
def main() -> None:
    ...
    print(f"APS 凝聚态下载面板已启动：{url}", flush=True)
